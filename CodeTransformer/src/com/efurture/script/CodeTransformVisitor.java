...
public class CodeTransformVisitor implements NodeVisitor {
    ...
    public boolean visit(AstNode node) {
      if (node.getParent() != null) {
    		CodeTransformer.log("visit xxx " + node.getClass()  + "  " + node.toSource() 
   				    + " " + node.getParent().getClass()); 
	}
     if (node instanceof NewExpression) {
    	 NewExpression expression = (NewExpression) node;
    	 
    	 CodeTransformer.log("new xxx " + expression.toSource()); 
    	 CodeTransformer.log("new xxx " + expression.getInitializer() + "  "   + 
  	  ...
  	  if(node instanceof FunctionCall && !(node instanceof NewExpression)){
  		     ...
  		     PropertyGet  get = (PropertyGet) node;
  		      ...
  		      if (!right.getIdentifier().contains(PREFIX)) {
  	  		     if (get.getParent() instanceof  ExpressionStatement) {
  					...
  					CodeTransformer.log("get xxx " + node.getClass()  + "  " + node.toSource() 
  					    + " " + node.getParent().getClass()); 
  				 }
			 }else{
				 CodeTransformer.log("get out " + node.getClass()  + "  " + node.toSource() 
	  					    + " " + node.getParent().getClass()); 
			 }
			 ...
			 if (node.getParent() instanceof NewExpression) {
			  ...
			  if (assignment.getLeft() instanceof PropertyGet) {
				  ...
				  CodeTransformer.log("Assignment " + node.toSource()  +   assignment.getParent().getClass());
			  }
			 
        }else {
        	CodeTransformer.log("end visit" + node.getClass()  + "  " + node.toSource());
  	  }
      return true;
    }
  }